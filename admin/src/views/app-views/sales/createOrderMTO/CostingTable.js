// import React, { useEffect, useMemo, useState } from 'react';
// import { Table, Typography, Button, Space, Popconfirm, message, Spin, Tag, InputNumber } from 'antd';
// import { EditOutlined, DeleteOutlined } from '@ant-design/icons';
// import ActionButtons from 'components/ActionButtons';

// const { Text } = Typography;

// const CostingTable = ({ costingItems = [], activeTab, onEdit, onDelete, loading = false, setMarkup, markup, allMarkups, onMarkupChange,drawing }) => {


//   useEffect(() => {
//     if (!allMarkups) return;
//     switch (activeTab) {
//       case 'materials':
//         setMarkup(allMarkups.material);
//         break;
//       case 'manhour':
//         setMarkup(allMarkups.manhour);
//         break;
//       case 'packing':
//         setMarkup(allMarkups.packing);
//         break;
//     }
//   }, [activeTab, allMarkups]);


//   const keyFor = (t) =>
//   t === "materials" ? "material" : t === "manhour" ? "manhour" : "packing";

// const baseline = Number(allMarkups?.[keyFor(activeTab)] ?? 0);

// const handleLocalChange = (val) => {
//   // AntD can send number | string | null
//   const raw = typeof val === "string" ? Number(val) : (val ?? 0);
//   const num = Number.isFinite(raw) ? Number(raw) : 0;
//    console.log('-------www',num)
//   // optimistic UI
//   setMarkup(num);

//   // send to parent as a real number
//   onMarkupChange?.(activeTab, num);
// };



//   // Action columns with edit and delete buttons
//   const actionColumn = {
//     title: 'Actions',
//     key: 'actions',
//     width: 160,
//     fixed: 'right',
//     render: (_, record) => (
//       <ActionButtons
//         showEdit
//         showDelete
//         showDeleteConfirm
//         onEdit={() => onEdit && onEdit(record)}
//         onDelete={() => onDelete && onDelete(record)}
//       />
//     ),
//   };

//   // Common columns jo sabhi tabs ke liye common hain
//   const commonColumns = [
//     {
//       title: 'Item Number',
//       dataIndex: 'itemNumber',
//       key: 'itemNumber',
//       width: 120,
//       fixed: 'left',
//       render: (text) => <Text strong>{text}</Text>,
//     },
//     {
//       title: 'Description',
//       dataIndex: 'description',
//       key: 'description',
//       width: 200,
//     },
//     {
//       title: 'UOM',
//       dataIndex: 'uom',
//       key: 'uom',
//       width: 80,
//       render: (uom) => uom ? uom.code : '-',
//     },
//     {
//       title: 'Qty',
//       dataIndex: 'quantity',
//       key: 'quantity',
//       width: 80,
//       align: 'right',
//       render: (value) => value?.toFixed(2) || '0.00',
//     },
//     {
//       title: 'Unit Price',
//       dataIndex: 'unitPrice',
//       key: 'unitPrice',
//       width: 100,
//       align: 'right',
//       render: (value) => `$${value?.toFixed(2) || '0.00'}`,
//     },
//     {
//       title: 'Ext Price',
//       dataIndex: 'extPrice',
//       key: 'extPrice',
//       width: 100,
//       align: 'right',
//       render: (value) => `$${value?.toFixed(2) || '0.00'}`,
//     },
//     {
//       title: 'Sales Price',
//       dataIndex: 'salesPrice',
//       key: 'salesPrice',
//       width: 100,
//       align: 'right',
//       render: (value) => `$${value?.toFixed(2) || '0.00'}`,
//     },
//   ];

//   // Material specific columns
//   const materialColumns = [
//     {
//       title: 'MPN',
//       dataIndex: 'mpn',
//       key: 'mpn',
//       width: 120,
//       render: (render) => render.MPN || '-',
//     },
//     {
//       title: 'Manufacturer',
//       dataIndex: 'manufacturer',
//       key: 'manufacturer',
//       width: 120,
//       render: (text) => text || '-',
//     },
//     {
//       title: 'SGA %',
//       dataIndex: 'sgaPercent',
//       key: 'sgaPercent',
//       width: 80,
//       align: 'right',
//       render: (value) => `${value || 0}%`,
//     },
//     {
//       title: 'Mat Burden %',
//       dataIndex: 'matBurden',
//       key: 'matBurden',
//       width: 100,
//       align: 'right',
//       render: (value) => `${value || 0}%`,
//     },
//     {
//       title: 'Lead Time',
//       dataIndex: 'leadTime',
//       key: 'leadTime',
//       width: 100,
//       align: 'right',
//       render: (value) => value ? `${value} weeks` : '-',
//     },
//   ];

//   // Manhour specific columns
//   const manhourColumns = [
//     {
//       title: 'Remarks',
//       dataIndex: 'remarks',
//       key: 'remarks',
//       width: 150,
//       render: (text) => text || '-',
//     },
//     {
//       title: 'Skill Level',
//       dataIndex: 'skillLevel',
//       key: 'skillLevel',
//       width: 120,
//       render: (skillLevel) => {
//         if (!skillLevel) return <Tag color="default">-</Tag>;

//         // Optional: pick color based on skillLevelName
//         let color = 'blue'; // default
//         switch (skillLevel.skillLevelName?.toLowerCase()) {
//           case 'beginner':
//             color = 'green';
//             break;
//           case 'intermediate':
//             color = 'orange';
//             break;
//           case 'expert':
//             color = 'red';
//             break;
//           default:
//             color = 'blue';
//         }

//         return <Tag color={color}>{skillLevel.skillLevelName} - {skillLevel?.currencyType?.symbol}{skillLevel?.rate}/{skillLevel?.type.code}</Tag>;
//       },
//     },
//   ];

//   // Packing specific columns
//   const packingColumns = [
//     {
//       title: 'MPN',
//       dataIndex: 'mpn',
//       key: 'mpn',
//       width: 120,
//       render: (mpn) => mpn.MPN || '-',
//     },
//     {
//       title: 'Max Burden %',
//       dataIndex: 'maxBurden',
//       key: 'maxBurden',
//       width: 100,
//       align: 'right',
//       render: (value) => `${value || 0}%`,
//     },
//     {
//       title: 'Freight %',
//       dataIndex: 'freightPercent',
//       key: 'freightPercent',
//       width: 80,
//       align: 'right',
//       render: (value) => `${value || 0}%`,
//     },
//   ];

//   // Active tab ke according columns set karen
//   const getColumns = () => {
//     const baseColumns = [...commonColumns];

//     switch (activeTab) {
//       case 'materials':
//         return [...baseColumns, ...materialColumns, actionColumn];
//       case 'manhour':
//         return [...baseColumns, ...manhourColumns, actionColumn];
//       case 'packing':
//         return [...baseColumns, ...packingColumns, actionColumn];
//       default:
//         return [...baseColumns, actionColumn];
//     }
//   };

//   // Calculate totals for current tab items
//   const calculateTotals = () => {
//     const filteredItems = costingItems.filter(item => {
//       switch (activeTab) {
//         case 'materials':
//           return item.quoteType === 'material';
//         case 'manhour':
//           return item.quoteType === 'manhour';
//         case 'packing':
//           return item.quoteType === 'packing';
//         default:
//           return true;
//       }
//     });

//     return {
//       quantity: filteredItems.reduce((sum, item) => sum + (item.quantity || 0), 0),
//       unitPrice: filteredItems.reduce((sum, item) => sum + (item.unitPrice || 0), 0),
//       extPrice: filteredItems.reduce((sum, item) => sum + (item.extPrice || 0), 0),
//       salesPrice: filteredItems.reduce((sum, item) => sum + (item.salesPrice || 0), 0),
//     };
//   };

//   const totals = calculateTotals();

//   const totalWithMarkup = useMemo(
//     () => totals.salesPrice + (totals.salesPrice * markup) / 100,
//     [totals, markup]
//   );


//   const columns = getColumns();

//   const filteredItems = costingItems.filter(item => {
//     switch (activeTab) {
//       case 'materials':
//         return item.quoteType === 'material';
//       case 'manhour':
//         return item.quoteType === 'manhour';
//       case 'packing':
//         return item.quoteType === 'packing';
//       default:
//         return true;
//     }
//   }).map((item, index) => ({
//     ...item,
//     key: item.id || item._id || index,
//   }));

//   if (loading) {
//     return (
//       <div style={{ textAlign: 'center', padding: '50px' }}>
//         <Spin size="large" />
//       </div>
//     );
//   }

//   return (
//     <div>
//       <Table
//         columns={columns}
//         dataSource={filteredItems}
//         pagination={false}
//         // size="small"
//         // scroll={{ x: 1200 }}
//         summary={() => (
//           <Table.Summary>
//             <Table.Summary.Row style={{ background: '#fafafa' }}>
//               <Table.Summary.Cell index={0} colSpan={3}>
//                 <Text strong>Sub-Total</Text>
//               </Table.Summary.Cell>
//               <Table.Summary.Cell index={1} align="right">
//                 {/* <Text>{totals.quantity.toFixed(2)}</Text> */}
//               </Table.Summary.Cell>
//               <Table.Summary.Cell index={2} align="right">
//                 {/* <Text>${totals.unitPrice.toFixed(2)}</Text> */}
//               </Table.Summary.Cell>
//               <Table.Summary.Cell index={3} align="right">
//                 <Text strong>${totals.extPrice.toFixed(2)}</Text>
//               </Table.Summary.Cell>
//               <Table.Summary.Cell index={4} colSpan={columns.length - 4} align="right">
//                 <Text strong>${totals.salesPrice.toFixed(2)}</Text>
//               </Table.Summary.Cell>
//             </Table.Summary.Row>

//             <Table.Summary.Row style={{ background: "#fafafa" }}>
//               <Table.Summary.Cell index={0} colSpan={columns.length - 1}>
//                 <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
//                   <Text strong>Markup (%)</Text>
//                  <InputNumber
//   min={baseline}
//   max={100}
//   value={markup}
//   onChange={handleLocalChange}
// />

//                 </div>
//               </Table.Summary.Cell>
//               <Table.Summary.Cell index={1} align="right">
//                 <Text strong>${totalWithMarkup.toFixed(2)}</Text>
//               </Table.Summary.Cell>
//             </Table.Summary.Row>
//           </Table.Summary>
//         )}
//       />
//     </div>
//   );
// };

// export default CostingTable;


import React, { useEffect, useMemo, useState } from 'react';
import { Table, Typography, Tag, InputNumber, Spin } from 'antd';
import ActionButtons from 'components/ActionButtons';
import { hasPermission } from 'utils/auth';

const { Text } = Typography;

const tabKey = (t) => (t === 'materials' ? 'material' : t === 'manhour' ? 'manhour' : 'packing');

const CostingTable = ({
  costingItems = [],
  activeTab,
  onEdit,
  onDelete,
  loading = false,
  currentMarkups,   // { material, manhour, packing }
  baseMarkups,      // { material, manhour, packing } baseline
  onMarkupChange,   // (tab, value) => persist to DB
}) => {
  const [localMarkup, setLocalMarkup] = useState(0);

  // sync local with parent whenever tab or markups change
  useEffect(() => {
    const k = tabKey(activeTab);
    setLocalMarkup(Number(currentMarkups?.[k] ?? 0));
  }, [activeTab, currentMarkups?.material, currentMarkups?.manhour, currentMarkups?.packing]);

  const baseline = Number(baseMarkups?.[tabKey(activeTab)] ?? 0);

  const handleLocalChange = (val) => {
    const n = typeof val === 'string' ? Number(val) : (val ?? 0);
    const num = Number.isFinite(n) ? Number(n) : 0;
    const clamped = Math.max(baseline, Math.min(100, num));
    setLocalMarkup(clamped);
    onMarkupChange?.(activeTab, clamped);
  };

  const actionColumn = {
    title: 'Actions',
    key: 'actions',
    width: 160,
    fixed: 'right',
    render: (_, record) => (
      <ActionButtons
        showEdit={hasPermission('sales.mto:deep_edit')}
        showDelete={hasPermission('sales.mto:deep_delete')}
        showDeleteConfirm
        onEdit={() => onEdit && onEdit(record)}
        onDelete={() => onDelete && onDelete(record)}
        showUpdate={hasPermission('sales.mto:deep_update')}
        onUpdate={() => console.log('------update')}
      />
    ),
  };

  const commonColumns = [
    {
      title: 'Item Number',
      dataIndex: 'itemNumber',
      key: 'itemNumber',
      width: 150,
      fixed: 'left',
      render: (text) => <Text strong>{text || '-'}</Text>,
    },
    { title: 'Description', dataIndex: 'description', key: 'description', width: 220 },
    {
      title: 'UOM',
      dataIndex: 'uom',
      key: 'uom',
      width: 80,
      render: (uom) => (uom?.code || uom?.name || '-'),
    },
    {
      title: 'Qty',
      dataIndex: 'quantity',
      key: 'quantity',
      width: 80,
      align: 'right',
      render: (v) => (Number(v || 0)).toFixed(2),
    },
    {
      title: 'Unit Price',
      dataIndex: 'unitPrice',
      key: 'unitPrice',
      width: 110,
      align: 'right',
      render: (v) => `$${(Number(v || 0)).toFixed(2)}`,
    },
    {
      title: 'Latest Price',
      dataIndex: 'unitPrice',
      key: 'unitPrice',
      width: 150,
      align: 'right',
      sorter: (a, b) => a.unitPrice - b.unitPrice, // enable sorting arrow
      render: (value) => (
        <div
          style={{
            // backgroundColor: '#ffeaea', // light red background
            color: '#d32f2f', // darker red text
            padding: '4px 8px',
            borderRadius: '6px',
            fontWeight: 600,
          }}
        >
          ${Number(value || 0).toFixed(2)}
        </div>
      ),
    }
    ,
    {
      title: 'Ext Price',
      dataIndex: 'extPrice',
      key: 'extPrice',
      width: 110,
      align: 'right',
      render: (v) => `$${(Number(v || 0)).toFixed(2)}`,
    },
    {
      title: 'Sales Price',
      dataIndex: 'salesPrice',
      key: 'salesPrice',
      width: 120,
      align: 'right',
      render: (v) => `$${(Number(v || 0)).toFixed(2)}`,
    },
  ];

  const materialColumns = [
    {
      title: 'MPN',
      dataIndex: 'mpn',
      key: 'mpn',
      width: 140,
      render: (mpn) => (typeof mpn === 'string' ? mpn : (mpn?.MPN || '-')),
    },
    {
      title: 'Manufacturer',
      dataIndex: 'manufacturer',
      key: 'manufacturer',
      width: 140,
      render: (t) => t || '-',
    },
    {
      title: 'SGA %',
      dataIndex: 'sgaPercent',
      key: 'sgaPercent',
      width: 90,
      align: 'right',
      render: (v) => `${Number(v || 0)}%`,
    },
    {
      title: 'Mat Burden %',
      dataIndex: 'matBurden',
      key: 'matBurden',
      width: 130,
      align: 'right',
      render: (v) => `${Number(v || 0)}%`,
    },
    {
      title: 'Lead Time',
      dataIndex: 'leadTime',
      key: 'leadTime',
      width: 110,
      align: 'right',
      render: (v) => v ? `${v} weeks` : '-',
    },
  ];

  const manhourColumns = [
    {
      title: 'Remarks',
      dataIndex: 'remarks',
      key: 'remarks',
      width: 160,
      render: (t) => t || '-',
    },
    {
      title: 'Skill Level',
      dataIndex: 'skillLevel',
      key: 'skillLevel',
      width: 180,
      render: (sl) => {
        if (!sl) return <Tag>-</Tag>;
        const lvl = (sl?.skillLevelName || '').toLowerCase();
        let color = 'blue';
        if (lvl === 'beginner') color = 'green';
        else if (lvl === 'intermediate') color = 'orange';
        else if (lvl === 'expert') color = 'red';
        return (
          <Tag color={color}>
            {sl?.skillLevelName ?? 'NA'} — {sl?.currencyType?.symbol}{sl?.rate}/{sl?.type?.code}
          </Tag>
        );
      },
    },
  ];

  const packingColumns = [
    {
      title: 'MPN',
      dataIndex: 'mpn',
      key: 'mpn',
      width: 140,
      render: (mpn) => (typeof mpn === 'string' ? mpn : (mpn?.MPN || '-')),
    },
    {
      title: 'Max Burden %',
      dataIndex: 'maxBurden',
      key: 'maxBurden',
      width: 120,
      align: 'right',
      render: (v) => `${Number(v || 0)}%`,
    },
    {
      title: 'Freight %',
      dataIndex: 'freightPercent',
      key: 'freightPercent',
      width: 110,
      align: 'right',
      render: (v) => `${Number(v || 0)}%`,
    },
  ];

  const getColumns = () => {
    const base = [...commonColumns];
    if (activeTab === 'materials') return [...base, ...materialColumns, actionColumn];
    if (activeTab === 'manhour') return [...base, ...manhourColumns, actionColumn];
    return [...base, ...packingColumns, actionColumn];
  };

  const filteredItems = useMemo(() => {
    const qt = activeTab === 'materials' ? 'material' : activeTab === 'manhour' ? 'manhour' : 'packing';
    return (costingItems || [])
      .filter((item) => (item?.quoteType || '').toLowerCase() === qt)
      .map((item, index) => ({ ...item, key: item.id || item._id || index }));
  }, [costingItems, activeTab]);

  const totals = useMemo(() => {
    const init = { quantity: 0, unitPrice: 0, extPrice: 0, salesPrice: 0 };
    return filteredItems.reduce((acc, it) => {
      acc.quantity += Number(it?.quantity || 0);
      acc.unitPrice += Number(it?.unitPrice || 0);
      acc.extPrice += Number(it?.extPrice || 0);
      acc.salesPrice += Number(it?.salesPrice || 0);
      return acc;
    }, init);
  }, [filteredItems]);

  const totalWithMarkup = useMemo(() => {
    const perc = Number(localMarkup || 0);
    return totals.salesPrice + (totals.salesPrice * perc) / 100;
  }, [totals.salesPrice, localMarkup]);

  if (loading) {
    return (
      <div style={{ textAlign: 'center', padding: 50 }}>
        <Spin size="large" />
      </div>
    );
  }

  const columns = getColumns();

  return (
    <div>
      <Table
        columns={columns}
        dataSource={filteredItems}
        pagination={false}
        summary={() => (
          <Table.Summary>
            <Table.Summary.Row >
              <Table.Summary.Cell index={0} colSpan={4}>
                <Text strong>Sub-Total</Text>
              </Table.Summary.Cell>
              <Table.Summary.Cell index={1} />
              <Table.Summary.Cell index={2} />
              <Table.Summary.Cell index={3} align="right">
                <Text strong>${totals.extPrice.toFixed(2)}</Text>
              </Table.Summary.Cell>
              <Table.Summary.Cell index={4} colSpan={columns.length - 4} align="right">
                <Text strong>${totals.salesPrice.toFixed(2)}</Text>
              </Table.Summary.Cell>
            </Table.Summary.Row>

            <Table.Summary.Row >
              <Table.Summary.Cell index={0} colSpan={columns.length - 1}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <Text strong>Markup (%)</Text>
                  <InputNumber
                    min={baseline}
                    max={100}
                    value={localMarkup}
                    onChange={handleLocalChange}
                  />
                  <Text type="secondary">Min (base): {baseline}%</Text>
                </div>
              </Table.Summary.Cell>
              <Table.Summary.Cell index={1} align="right">
                <Text strong>${totalWithMarkup.toFixed(2)}</Text>
              </Table.Summary.Cell>
            </Table.Summary.Row>
          </Table.Summary>
        )}
      />
    </div>
  );
};

export default CostingTable;
