...
const DrawingDetails = () => {
    ...
    const drawingDetails = [
        ...
        { key: 'currency', label: 'Currency', value: drawing?.currency?.code || drawing?.projectId?.currency || "-", bold: true },
        { key: 'unitPrice', label: 'Unit Price', value: `$${grandTotalWithMarkup.toFixed(2)}`, bold: true },
    ...
    return (
        <div style={{ padding: 24, background: '#fbf7f7ff', minHeight: '100vh' }}>
            <Card>
                <Card
                    title={<Text strong style={{ fontSize: 16 }}>Drawing: {drawing?.projectId?.projectName || "N/A"}</Text>}
                        ...
                        {drawingDetails.map((item) => (
                            <Col xs={24} sm={12} md={8} lg={6} key={item.key}>
                                <div>
                                    ...
                                    <br />
                                    <Text strong={item.bold} style={{ fontSize: 14 }}>{item.value}</Text>
                                </div>
                            </Col>
                        ))}
                    </Row>
