...
import { CalendarOutlined, FileExcelOutlined, FileWordOutlined, PlusOutlined, SearchOutlined, UserOutlined } from "@ant-design/icons";
...
const CreateQuote = () => {
          ...
          <Space direction="vertical" style={{ width: '100%' }} size={0}>
            ...
            <Space size="middle" style={{ marginTop: 4 }}>
              <Text strong style={{ fontSize: '14px' }}>
                Unit: ${(record.items?.[0]?.unitPrice || 0).toFixed(2)}
              </Text>
              <Text strong style={{ fontSize: '14px' }}>
  ...
  const fetchQuotes = async () => {
    try {
      ...
      console.log('=====', res)
      if (res.success) setQuotesData(res.data);
    } catch (err) { console.error(err); }
  };
